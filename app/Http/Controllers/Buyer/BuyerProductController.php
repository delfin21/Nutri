...
class BuyerProductController extends Controller
{
    ...
    {
$query = Product::with(['farmer']) // ✅ include farmer
...
$topProducts = Product::with(['farmer'])
    ->withAvg('reviews', 'rating')
    ->withCount('reviews')
...
$discoverProducts = Product::with(['farmer'])
    ->withAvg('reviews', 'rating')
    ->withCount('reviews')
    ->latest()
    ->take(12)
...
public function addToCart(Request $request, $productId)
{
    ...
    public function categoryProducts($slug)
    {
        ...
        $discoverProducts = Product::withAvg('reviews', 'rating')
            ->withCount('reviews')
            ->where('category', $category)
            ->latest()
            ->take(8)
...
$query = Product::with(['farmer'])
    ...
    // Discover Products (Optional: You can filter this too if you want)
$discoverProducts = Product::with(['farmer'])
    ->withAvg('reviews', 'rating')
    ->withCount('reviews')
    ->where('category', $normalizedCategory)
...
$query = Product::with(['farmer'])
    ...
    // Include categories (customize if using dynamic list)
    $categories = ['fruits', 'vegetable', 'grains', 'spices'];
...
{
    $product = Product::findOrFail($productId); // ✅ Move this up
    ...
    // Temporarily store this product as "buy now" session
    session([
        'buy_now' => [
